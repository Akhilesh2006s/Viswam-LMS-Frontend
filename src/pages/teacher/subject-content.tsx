...
import { fetchTeacherLearningPaths } from '@/lib/teacher-api';
...
function flattenTeacherContentRows(
  rows: Awaited<ReturnType<typeof fetchTeacherLearningPaths>>,
): LearningPathItem[] {
  const lists = rows.map((row) => (row.asliPrepContent || []) as LearningPathItem[]);
  return mergeLearningPathItems(...lists);
}
...
export default function TeacherSubjectContent() {
  ...
  const [contents, setContents] = useState<LearningPathItem[]>([]);
  ...
  const fetchSubjectContent = async (subjectId: string, mergeSubjectIds: string[]) => {
    ...
    try {
      ...
      const contentPromises = subjectIds.map((id) => fetchTeacherLearningPaths(id));

      const [subjectResponse, ...contentRows] = await Promise.all([
        subjectPromise,
        ...contentPromises,
      ]);
      ...
      const lists = contentRows.map((rows) => flattenTeacherContentRows(rows));
      const merged = mergeLearningPathItems(...lists);
      setContents(merged);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-sky-50/80 to-teal-50/50">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div className="mb-6">
          <Button
            variant="ghost"
            onClick={() => setLocation('/teacher/dashboard?tab=content')}
            className="mb-4"
          >
        ...
        <Card className="bg-white/90 backdrop-blur-xl shadow-lg border border-slate-200/80">
          ...
          <CardContent>
            <LearningPathMaterials
              ...
              emptyMessage="No textbooks or videos for this subject yet. Content appears when your school admin publishes materials for your assigned products."
            />
          </CardContent>
        </Card>
      </div>
